...
function StatusBadge({ status }) {
  const map = {
    ...
    live: { label: "جارية", cls: "bg-destructive/10 text-destructive" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map((t) => {
          ...
          return (
            <Link key={t.id} to={`/tournaments/${t.id}`} data-testid={`tournament-${t.id}`} className="bg-surface border b-soft rounded-lg p-5 hover:border-gold-500/30 transition fade-in">
