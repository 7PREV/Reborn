...
function AddForm({ onCreated }) {
  ...
  const searchLinkedAccount = async (q) => {
    setAccountQuery(q);
  ...
  const pickUser = (u) => {
    setForm((prev) => ({
      ...prev,
      player_name: prev.player_name || u.username,
      player_user_id: u.id,
      player_email: u.email,
    }));
    setAccountQuery(`${u.username} • ${u.email}`);
    setSearchResults([]);
  };

  const clearLinkedAccount = () => {
    setForm((prev) => ({ ...prev, player_user_id: "", player_email: "" }));
    setAccountQuery("");
    setSearchResults([]);
  };
  ...
  return (
    <form onSubmit={submit} className="bg-surface border b-soft rounded-xl p-5 space-y-3" data-testid="blacklist-form">
      ...
      <div className="relative">
        <label className="text-xs uppercase tracking-widest text-white/50 mb-1 block">ربط الحساب (اختياري)</label>
        <input
          data-testid="bl-account-search"
          value={accountQuery}
          onChange={(e) => searchLinkedAccount(e.target.value)}
          placeholder="ابحث عن الحساب بالبريد أو اسم المستخدم"
          className="w-full bg-background border b-soft rounded-md px-3 py-2 outline-none focus:border-destructive/50 text-sm"
        />
        {form.player_user_id && (
          <div className="mt-2 flex items-center justify-between gap-2 rounded-md border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300">
            <div className="truncate">تم ربط الحساب: {form.player_email || form.player_user_id}</div>
            <button type="button" onClick={clearLinkedAccount} className="text-white/70 hover:text-white">إلغاء الربط</button>
          </div>
        )}
        <input
          ...
          placeholder="اسم اللاعب (يدوي أو من الحساب المرتبط)"
          className="mt-3 w-full bg-background border b-soft rounded-md px-3 py-2 outline-none focus:border-destructive/50 text-sm"
        />
