...
def _checkin(token, clan_id):
    h = {"Authorization": f"Bearer {token}"}
    r = requests.post(f"{API}/clans/{clan_id}/attendance/checkin", headers=h)
    assert r.status_code == 200, r.text
    return r.json()


def _finish_match_for_a(mid, token_a, token_b, clan_a_id):
    h_a = {"Authorization": f"Bearer {token_a}"}
    h_b = {"Authorization": f"Bearer {token_b}"}
    for idx in (0, 2):
        r1 = requests.post(f"{API}/matches/{mid}/vote-map", headers=h_a, json={"map_index": idx, "winner_clan_id": clan_a_id})
        assert r1.status_code == 200, r1.text
        r2 = requests.post(f"{API}/matches/{mid}/vote-map", headers=h_b, json={"map_index": idx, "winner_clan_id": clan_a_id})
...
def test_all_checked_in_members_receive_win_loss():
    _, ua, ta = _register("att_all_a")
    _, ub, tb = _register("att_all_b")
    ca = _make_clan(ta, "ataa")
    cb = _make_clan(tb, "atbb")
    a_members = _fill_clan(ta, ca["id"], count=6)  # leader + 6 => 7 total members
    b_members = _fill_clan(tb, cb["id"], count=6)

    # Check in everyone on both sides (all-inclusive rule)
    _checkin(ta, ca["id"])
    _checkin(tb, cb["id"])
    for t in a_members:
        _checkin(t, ca["id"])
    ...
    h_a = {"Authorization": f"Bearer {ta}"}
    h_b = {"Authorization": f"Bearer {tb}"}
    ch = requests.post(f"{API}/clans/{cb['id']}/challenge", headers=h_b, json={"opponent_clan_id": ca["id"]})
    assert ch.status_code == 200, ch.text
    acc = requests.post(f"{API}/challenges/{ch.json()['id']}", headers=h_a, json={"action": "accept"})
    assert acc.status_code == 200, acc.text
    mid = acc.json()["match"]["id"]

    _finish_match_for_a(mid, ta, tb, ca["id"])
    final_match = requests.get(f"{API}/matches/{mid}").json()
    assert final_match["status"] == "finished"

    # Leader + every checked-in substitute on winner side should get +1 win
    win_ids = [ua["id"]]
    for tok in a_members:
        me = requests.get(f"{API}/auth/me", headers={"Authorization": f"Bearer {tok}"})
        assert me.status_code == 200
        win_ids.append(me.json()["id"])
    for uid in win_ids:
        pu = requests.get(f"{API}/users/{uid}")
        assert pu.status_code == 200
        assert pu.json().get("wins", 0) >= 1

    # Leader + every checked-in substitute on loser side should get +1 loss
    loss_ids = [ub["id"]]
    for tok in b_members:
        me = requests.get(f"{API}/auth/me", headers={"Authorization": f"Bearer {tok}"})
        assert me.status_code == 200
        loss_ids.append(me.json()["id"])
    for uid in loss_ids:
        pu = requests.get(f"{API}/users/{uid}")
        assert pu.status_code == 200
        assert pu.json().get("losses", 0) >= 1


def test_mvp_voting_flow_and_clan_leaderboard():
    _, _, ta = _register("mvp_a")
    _, _, tb = _register("mvp_b")
    ca = _make_clan(ta, "mvpa")
    cb = _make_clan(tb, "mvpb")
    a_members = _fill_clan(ta, ca["id"], count=5)
    _fill_clan(tb, cb["id"], count=5)

    # Keep MVP electorate small: two attendees from clan A only
    _checkin(ta, ca["id"])
    _checkin(a_members[0], ca["id"])
    ...
    s1 = requests.get(f"{API}/matches/{mid}/mvp-status", headers=h_a)
    assert s1.status_code == 200, s1.text
    assert s1.json().get("can_vote") is True
    teammates = s1.json().get("eligible_teammates") or []
    assert teammates, "Expected at least one eligible teammate for MVP vote"
    teammate_id = teammates[0]["id"]

    v1 = requests.post(f"{API}/matches/{mid}/mvp-vote", headers=h_a, json={"player_id": teammate_id})
    assert v1.status_code == 200, v1.text

    h_a2 = {"Authorization": f"Bearer {a_members[0]}"}
    leader_me = requests.get(f"{API}/auth/me", headers=h_a)
    assert leader_me.status_code == 200
    leader_id = leader_me.json()["id"]
    v2 = requests.post(f"{API}/matches/{mid}/mvp-vote", headers=h_a2, json={"player_id": leader_id})
    assert v2.status_code == 200, v2.text

    s2 = requests.get(f"{API}/matches/{mid}/mvp-status", headers=h_a)
    assert s2.status_code == 200
    winner = s2.json().get("winner")
    assert winner and winner.get("id"), "MVP winner should be finalized once all eligible attendees vote"

    lb = requests.get(f"{API}/clans/{ca['id']}/mvp-leaderboard")
    assert lb.status_code == 200
    assert isinstance(lb.json(), list)
    assert any(row.get("mvp_count", 0) >= 1 for row in lb.json())
