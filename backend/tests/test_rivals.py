...
def _add_members_to_clan(leader_token: str, clan_id: str, count: int = 5):
    """Add `count` extra members to a clan via join requests (leader counts as 1 already)."""
    h_leader = {"Authorization": f"Bearer {leader_token}"}
    for i in range(count):
        s_m, u_m, t_m, _ = _register(f"fill{i}_{uuid.uuid4().hex[:4]}")
        h_member = {"Authorization": f"Bearer {t_m}"}
        jr = s_m.post(f"{API}/clans/{clan_id}/join-request", headers=h_member)
        assert jr.status_code == 200, jr.text
        reqs = requests.get(f"{API}/clans/{clan_id}/requests", headers=h_leader)
        assert reqs.status_code == 200, reqs.text
        target = next((r for r in reqs.json() if r.get("user_id") == u_m["id"] and r.get("status") == "pending"), None)
        assert target, f"join request missing for user {u_m['id']}"
        acc = requests.post(
            f"{API}/clans/{clan_id}/requests/{target['id']}",
            headers=h_leader,
            json={"action": "accept"},
        )
...
def test_toggle_plus():
    ...
    # Must fail before clan reaches minimum members
    r = s.post(f"{API}/me/plus", headers=h)
    assert r.status_code == 400
    assert "6 لاعبين" in r.json().get("detail", "")

    # Create clan and fill to 6 total members, then activation should pass
    name = f"TEST_plusclan_{uuid.uuid4().hex[:5]}"
    tag = f"P{uuid.uuid4().hex[:4]}"
    cr = requests.post(f"{API}/clans", headers=h, json={"name": name, "tag": tag, "description": "plus"})
    assert cr.status_code == 200, cr.text
    _add_members_to_clan(t, cr.json()["id"], count=5)
...
def test_clan_detail_plus_limits():
    ...
    assert r.status_code == 200, r.text
    cid = r.json()["id"]
    _add_members_to_clan(t, cid, count=5)
    plus = requests.post(f"{API}/me/plus", headers=h)
